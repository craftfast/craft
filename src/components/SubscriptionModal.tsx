...
export default function SubscriptionModal({
  ...
  const handleUpgradeToPro = async () => {
    try {
      ...
      // Default to 100 credits/month ($25/mo) for Pro tier
      const response = await fetch("/api/billing/upgrade-to-pro", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ monthlyCredits: 100 }),
      });
    ...
    <DialogPrimitive.Root open={isOpen} onOpenChange={handleClose}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            "fixed left-[50%] top-[50%] z-[100001] flex flex-col w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-2xl max-h-[90vh] overflow-hidden p-0"
          )}
        >
          {/* Close button */}
